...
export default async function Home() {
  ...
  try {
    const [chiefEditorData, titlsData, importantNewPage, articles] = await Promise.all([
      chiefEditor({url}),
      titls({url}),
      importantNew({url}),
      Articles({url})
    ...
    return (
      <>
      <div className="flex flex-col md:flex-row mx-auto  max-w-7xl items-start mt-4  justify-between">
            ...
            {chiefEditorData.map((item: any) => (
              <li key={item.id} className="flex flex-col items-center">
                <div className="flex flex-row">
                   <img src="/avatar/1401120109413719927095974-300x209.jpg" className="rounded-full w-24 h-24"/>
                   <div className="mr-2 ">
                     <p className="text-xs">احسان صالحی</p>
                     <Link href={`${item.id}`}><p className="pt-2 hover:text-red-500 text-xl">{item.title}</p></Link>
                    </div>
                </div>
      ...
      <div className="max-w-7xl mx-auto border-t-4 border-[#898989] mt-2">
                ...
                >
                  ...
                  <div className="mr-4">
                    ...
                    <p className="text-sm ">
                      ...
                      A praesentium excepturi et accusantium reiciendis,
                      corporis quisquam inventore dolor, vero autem perferendis
                      adipisci quasi eveniet soluta ullam, consectetur dolores
                      iure. Quasi.
                    </p>
                  </div>
                </li>
